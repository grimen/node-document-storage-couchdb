...
CouchDB.defaults = CouchDB.defaults || {};
CouchDB.defaults.url = Storage.env('COUCHDB_URL') || 'http://localhost:5984/{db}-{env}'.assign({
  db: 'default',
  env: process.env.NODE_ENV || 'development'
});
CouchDB.defaults.options = {
  client: {
    cache: true,
    raw: false
  }
...
CouchDB.prototype.connect = function() {
  ...
  self._connect(function() {
    ...
    var db = self.client.database(self.resource().trimmed.db);
...
CouchDB.prototype.set = function() {
  ...
  self._set(arguments, function(key_values, options, done, next) {
    ...
    key_values.each(function(key, value) {
      ...
      db.save(resource.key, value, function(error, response) {
...
CouchDB.prototype.get = function() {
  ...
  self._get(arguments, function(keys, options, done, next) {
    ...
    keys.each(function(key) {
      ...
      db.get(resource.key, function(error, response) {
        if (error && error.error === 'not_found') {
          error = null;
          response = null;
        }
...
CouchDB.prototype.del = function() {
  ...
  self._del(arguments, function(keys, options, done, next) {
    ...
    keys.each(function(key) {
      ...
      db.get(resource.key, function(get_error, get_response) {
